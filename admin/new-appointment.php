...
<?php
$ret=mysqli_query($con,"select bookings.name,bookings.ID, bookings.Services, bookings.beautician, bookings.date, bookings.timeslot, users.id from bookings INNER JOIN users ON bookings.name=users.name where Status='' GROUP BY ApplyDate order by date DESC ");
$cnt=1; 
